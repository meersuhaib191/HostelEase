...
if (isset($_POST['submit'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Send greeting email to the student
        $mail = new PHPMailer();
        $mail->isSMTP();
        $mail->Host       = 'smtp.gmail.com';  // Gmail SMTP server
        $mail->SMTPAuth   = true;
        $mail->Username   = 'meersuhaib191@gmail.com'; // Your Gmail address
        $mail->Password   = 'yxtxcnzygdzrpgim';           // Your Gmail password or app-specific password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;  // Port for TLS

        $mail->setFrom('meersuhaib191@gmail.com', $hostel_name); // Sender's email and hostel name
        $mail->addAddress($emailid); // Recipient's email
        $mail->isHTML(true);

        $mail->Subject = "Welcome to $hostel_name!";
        $mail->Body = "<h1>Welcome, $student_name!</h1><p>Dear $student_name,</p><p>Thank you for registering with $hostel_name. We are excited to have you!</p>";

        if ($mail->send()) {
            echo "<script>alert('Student has been registered and greeting email sent!');</script>";
        } else {
            echo "<script>alert('Student has been registered, but email could not be sent.');</script>";
        }

        // Redirect to avoid multiple submissions on refresh
